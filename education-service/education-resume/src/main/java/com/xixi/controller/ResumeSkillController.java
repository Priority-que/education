...
public class ResumeSkillController {
    ...
    @MethodPurpose("7.1：新增技能")
    @PostMapping("/create")
    public Result create(
            @RequestBody ResumeSkillCreateDto dto,
            @RequestHeader(value = AuthHeaderConstants.HEADER_USER_ID, required = false) String userIdHeader
    ) {
        return resumeSkillService.create(dto, resolveStudentId(userIdHeader));
    }

    @MethodPurpose("7.2：修改技能")
    @PutMapping("/update")
    public Result update(
            @RequestBody ResumeSkillUpdateDto dto,
            @RequestHeader(value = AuthHeaderConstants.HEADER_USER_ID, required = false) String userIdHeader
    ) {
        return resumeSkillService.update(dto, resolveStudentId(userIdHeader));
    }

    @MethodPurpose("7.3：删除技能")
    @DeleteMapping("/delete/{id}")
    public Result delete(
            @PathVariable Long id,
            @RequestHeader(value = AuthHeaderConstants.HEADER_USER_ID, required = false) String userIdHeader
    ) {
        return resumeSkillService.delete(id, resolveStudentId(userIdHeader));
    }

    @MethodPurpose("7.4：查询技能列表")
    @GetMapping("/list/{resumeId}")
    public Result list(
            @PathVariable Long resumeId,
            @RequestHeader(value = AuthHeaderConstants.HEADER_USER_ID, required = false) String userIdHeader
    ) {
        return Result.success(resumeSkillService.listByResumeId(resumeId, resolveStudentId(userIdHeader)));
    }
